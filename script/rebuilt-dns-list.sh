...
allow=(
  ...
  #anti-AD/whiterule
  "https://raw.githubusercontent.com/privacy-protection-tools/anti-AD/adlist-maker/scripts/origin-files/whiterule-src-easylist.txt"
  "https://raw.githubusercontent.com/privacy-protection-tools/dead-horse/master/anti-ad-white-list.txt"
)
...
#
#F----
cat base-src-easylist.txt base-src-hosts.txt wildcard-src-easylist.txt ../../allow-domains-list.txt | grep -vE ']|@|!|\/' \
| sed 's/[ ]//g' | sed '/^$/d' \
...
cat dns.txt|grep -v '@'|grep -Po "(?<=\|\|).+(?=\^)"| grep -v "\*" >> ./clash.list
...
# 添加规则统计到'clash.txt'
rule_count_clash=$(grep -vc '^#' "clash.list")
sed -i "3i # $rule_count_clash Rules" "clash.list"

# 转换clash.list为clash.mrs(mihomo)
...
chmod +x mihomo
./mihomo convert-ruleset domain text clash.list clash.mrs
wait
